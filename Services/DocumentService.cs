...
namespace LilyRagPractices.Services
{
    ...
    public sealed class SearchResult
    {
        ...
        {
            _conn = config.GetConnectionString("RagDb") ?? throw new ArgumentNullException("RagDb connection string is not configured.");
        }
        ...
        public async Task InsertDocumentAsync(string filename, string content, float[] embedding)
        {
            ...
            {
                throw new InvalidOperationException($"Embedding must be {ExpectedDimensions} dimensions. Got {embedding?.Length ?? 0}.");
            }
        ...
        public async Task<List<SearchResult>> SearchAsync(float[] queryEmbedding, int limit = 5)
        {
            ...
            {
                throw new InvalidOperationException($"Query embedding must be {ExpectedDimensions} dimensions. Got {queryEmbedding?.Length ?? 0}.");
            }
